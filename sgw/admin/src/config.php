...
define('SECURITY_TOKEN_BLOCK_JSON', '/etc/nginx/subscribe/security_token_block.json');
...
define('TOKEN_BLOCK_CONF',  '/etc/nginx/subscribe/token_block.conf');
...
function normalize_security_settings(array $s): array {
    ...
    if (!in_array($action, ['blacklist_ip', 'block_token', 'both'], true)) {
        $action = DEFAULT_SECURITY_RELATION_ACTION;
    }
...
function safe_token_value(string $token): string {
    $token = trim($token);
    if ($token === '' || strlen($token) > 256 || preg_match('/[\r\n{};"`]/', $token)) return '';
    return $token;
}

function read_security_token_blocks(): array {
    $entries = read_json_array_file(SECURITY_TOKEN_BLOCK_JSON);
    $out = [];
    foreach ($entries as $e) {
        $token = safe_token_value((string)($e['token'] ?? ''));
        if ($token === '') continue;
        $out[$token] = [
            'token' => $token,
            'comment' => safe_comment((string)($e['comment'] ?? '')),
            'added_at' => safe_comment((string)($e['added_at'] ?? '')),
        ];
    }
    return array_values($out);
}

function write_security_token_blocks(array $entries, ?string $subscribePath = null): bool {
    $normalized = [];
    foreach ($entries as $e) {
        $token = safe_token_value((string)($e['token'] ?? ''));
        if ($token === '') continue;
        $normalized[$token] = [
            'token' => $token,
            'comment' => safe_comment((string)($e['comment'] ?? '')),
            'added_at' => safe_comment((string)($e['added_at'] ?? date('Y-m-d H:i'))),
        ];
    }
    $list = array_values($normalized);
    $r1 = file_put_contents(SECURITY_TOKEN_BLOCK_JSON, json_encode($list, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), LOCK_EX);
    $r2 = write_token_block_conf($list, $subscribePath);
    return $r1 !== false && $r2;
}

function write_token_block_conf(array $entries, ?string $subscribePath = null): bool {
    $base = normalize_subscribe_path($subscribePath ?? ($GLOBALS['_sg']['subscribe_path'] ?? null));
    $baseRe = nginx_literal_regex($base);
    $lines = [
        '# Token block map - generated by SubSieve security monitor',
        'map $request_uri $is_blocked_token {',
        '    default 0;',
    ];
    $seen = [];
    foreach ($entries as $e) {
        ...
        $seen[$token] = true;
        $tokenRe = nginx_literal_regex($token);
        $enc = rawurlencode($token);
        $encRe = $enc !== $token ? '|' . nginx_literal_regex($enc) : '';
        $lines[] = '    ~*"(^' . $baseRe . '/(' . $tokenRe . $encRe . ')(?:$|[/?#])|[?&]token=(' . $tokenRe . $encRe . ')(?:&|$))" 1;';
    }
    $lines[] = '}';
    $lines[] = '';
    return file_put_contents(TOKEN_BLOCK_CONF, implode("\n", $lines), LOCK_EX) !== false;
}

function write_blacklist_entries(array $entries): bool {
